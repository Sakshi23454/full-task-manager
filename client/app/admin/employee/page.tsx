...
const AdminEmployee = () => {
    ...
    const handleRemove = async (data: REMOVE_EMPLOYEE_REQUEST) => {
        try {
            ...
            toast.success("employee account permanent delete success")
        } catch (error) {
            console.log(error)
            toast.error("unable to delete")
        }
                                ...
                                <td>
                                    {
                                        item.isDelete
                                            ? <div>
                                                <button disabled={restoreLoading} onClick={() => handleRestore({ _id: item._id, isDelete: false })} className='btn btn-warning btn-sm'>
                                                    {
                                                        restoreLoading
                                                ...
                                                <button onClick={e => setSelectedEmp(item._id)} data-bs-toggle="modal" data-bs-target="#deleteModal" className='ms-2 btn btn-sm btn-danger'>Permanent delete</button>
                                            </div>
                                            ...
                                            : <div>
                                                ...
                                                <button disabled={deleteLoading} onClick={() => handleDelete({ _id: item._id })} className='btn btn-sm btn-outline-danger ms-2'>
                                                    {
                                                        deleteLoading
                                                            ? <div className="spinner spinner-border-sm"></div>
                ...
                <div className="modal-content">
                    ...
                    <div className="modal-body">
                        <h6 className='text-danger'>
                            you are about to permanently delete this employee. Do you wish to continue ?
                        </h6>
